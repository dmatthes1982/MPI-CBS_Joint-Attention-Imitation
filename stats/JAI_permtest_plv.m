...
fileList     = dir([srcPath, 'JAI_d*_07a_plv2Hz_*.mat']);
...
for dyad=1:1:numOfFiles
  fileListCopy{dyad} = strsplit(fileList{dyad}, '07a_plv2Hz_');
...
fileList     = dir([srcPath 'JAI_d*_07a_plv' passband '_' sessionStr ...
                    '.mat']);
fileList     = struct2cell(fileList);
...
while selection == false
  ...
  if ~all(isstrprop(identifier{1}, 'alphanum'))                             % check if identifier is valid
    ...
    matFile = [desPath 'JAI_plvStats_' identifier{1} '_' sessionStr ...     % build filename
                '.mat'];
