...
cprintf([0,0.6,0], '<strong>Export of ITPC results (general script)</strong>\n');
...
condMark  = cellfun(@(x) sprintf('S%3d', x), ...                            % extract condition identifiers
                    num2cell(generalDefinitions.condNumITPC), ...
                    'UniformOutput', false);
...
labelOrig = data_itpc.part1.label;                                          % extract channel names
...
clear data_itpc numOfChan part selection x prompt_string
...
desPath = [path 'DualEEG_JAI_results/ITPC_export/general/' sessionStr '/']; % destination path
...
while selection == false
  ...
  if ~all(isstrprop(identifier{1}, 'alphanum'))                             % check if identifier is valid
    ...
    xlsFile = [desPath 'ITPC_general_export_' identifier{1} '_' ...         % build filename
              sessionStr '.xls'];
...
% -------------------------------------------------------------------------
fprintf('<strong>Import of ITPC values...</strong>\n\n');
f = waitbar(0,'Please wait...');

for dyad = 1:1:numOfFiles
  load([srcPath fileList{dyad}]);                                           % load data
  
  if any(~strcmp(data_itpc.part1.label, labelOrig))
  ...
  for trl=1:1:numOfTrials
    waitbar(((dyad-1)*numOfTrials + trl)/(numOfFiles * numOfTrials), ...
                  f, 'Please wait...');
    loc_trl = ismember(data_itpc.part1.trialinfo, condNum(trl));            % participant 1
    if any(loc_trl)
      itpcPart1 = nanmean(data_itpc.part1.itpc{loc_trl}, 3);
      loc_freq  = ismember(data_itpc.part1.freq, freqNum);
      loc_chan  = ismember(data_itpc.part1.label, label);
      itpcPart1 = itpcPart1(loc_chan, loc_freq);
      row = 2*dyad - 1;
      
      switch mode
        case 0 % average & cluster
          itpcPart1 = reshape(itpcPart1, [], 1);
          Tdata(row, trl + 1) = {nanmean(itpcPart1)};
        case 1 % singleFreq & cluster
          start = (trl - 1) * numOfFreq + 2;
          stop  = start + numOfFreq - 1;
          Tdata(row ,start:stop) = num2cell(nanmean(itpcPart1, 1));
        case 2 % average & singleChan
        ...
        case 3 % singelFreq & singleChan
          ...
          itpcPart1 = transpose(itpcPart1);
          itpcPart1 = reshape(itpcPart1,[],1);
          itpcPart1 = transpose(itpcPart1);
          Tdata(row ,start:stop) = num2cell(itpcPart1);
      end
    end
    
    loc_trl = ismember(data_itpc.part2.trialinfo, condNum(trl));            % participant 2
    if any(loc_trl)
      itpcPart2 = nanmean(data_itpc.part2.itpc{loc_trl}, 3);
      loc_freq  = ismember(data_itpc.part2.freq, freqNum);
      loc_chan  = ismember(data_itpc.part2.label, label);
      itpcPart2 = itpcPart2(loc_chan, loc_freq);
      row = 2*dyad;
      
      switch mode
        case 0 % average & cluster
          itpcPart2 = reshape(itpcPart2, [], 1);
          Tdata(row, trl + 1 ) = {nanmean(itpcPart2)};
        case 1 % singleFreq & cluster
          start = (trl - 1) * numOfFreq + 2;
          stop  = start + numOfFreq - 1;
          Tdata(row ,start:stop) = num2cell(nanmean(itpcPart2, 1));
        case 2 % average & singleChan
          start = (trl - 1) * numOfChan + 2;
          stop  = start + numOfChan - 1;
          Tdata(row ,start:stop) = num2cell(transpose(...
        ...
        case 3 % singelFreq & singleChan
          start = (trl - 1) * (numOfChan * numOfFreq) + 2;
          stop  = start + (numOfChan * numOfFreq) - 1;
          itpcPart2 = transpose(itpcPart2);
          itpcPart2 = reshape(itpcPart2,[],1);
          itpcPart2 = transpose(itpcPart2);
          Tdata(row ,start:stop) = num2cell(itpcPart2);
      end
    end
  end
...
clear f dyad numOfFiles srcPath fileList labelOrig dyads trl loc_chan ...
      numOfTrials loc_trl condNum data_itpc loc_freq start stop mode ...
      freqNum itpcPart1 itpcPart2 label row numOfChan numOfFreq

% -------------------------------------------------------------------------
% Export itpc table into spreadsheet
% -------------------------------------------------------------------------
fprintf('<strong>Export of ITPC table into a xls spreadsheet...</strong>\n');
