...
cprintf([0,0.6,0], '<strong>Export of ITPC results (seperated by single and dual conditions)</strong>\n');
...
srcPath = [srcPath  '09b_itpcavg/'];

fileList     = dir([srcPath, 'JAI_d*_09b_itpcavg_*.mat']);
...
for dyad=1:1:numOfFiles
  fileListCopy{dyad} = strsplit(fileList{dyad}, '09b_itpcavg_');
...
xlsFile = [desPath 'ITPC_singledual_conditions_export_' sessionStr '.xls']; % build file name
...
% -------------------------------------------------------------------------
fileList     = dir([srcPath 'JAI_d*_09b_itpcavg_' sessionStr '.mat']);
fileList     = struct2cell(fileList);
...
label         = label(loc);
label_No_S    = cellfun(@(x) [x '_0S'], label, 'UniformOutput', false);     % generate column titles for each kind of entrainment
label_No_D    = cellfun(@(x) [x '_0D'], label, 'UniformOutput', false);
label_2Hz_S   = cellfun(@(x) [x '_2S'], label, 'UniformOutput', false);
label_2Hz_D   = cellfun(@(x) [x '_2D'], label, 'UniformOutput', false);
label_10Hz_S  = cellfun(@(x) [x '_10S'], label, 'UniformOutput', false);
label_10Hz_D  = cellfun(@(x) [x '_10D'], label, 'UniformOutput', false);
label_20Hz_S  = cellfun(@(x) [x '_20S'], label, 'UniformOutput', false);
label_20Hz_D  = cellfun(@(x) [x '_20D'], label, 'UniformOutput', false);

numOfChan = 8 * length(label);
...
col{5}    = col{4} + length(label);
col{6}    = col{5} + length(label);
col{7}    = col{6} + length(label);
col{8}    = col{7} + length(label);

cell_array      = num2cell(NaN(numOfPart, numOfChan + 1));
cell_array(:,1) = {'unknown'};
T               = cell2table(cell_array);                                   % create template table
T.Properties.VariableNames = ['participant' label_No_S' label_No_D' ...
                                label_2Hz_S' label_2Hz_D' label_10Hz_S' ...
                                label_10Hz_D' label_20Hz_S' label_20Hz_D'];
T_NB2   = T;                                                                % create table for each narrow band
T_NB10  = T;
T_NB20  = T;

clear T label_No_S label_No_D label_2Hz_S label_2Hz_D label_10Hz_S ...
      label_10Hz_D label_20Hz_S label_20Hz_D numOfPart numOfChan ...
      cell_array sessionStr loc

% -------------------------------------------------------------------------
% import itpc values into tables
% -------------------------------------------------------------------------
condition  = {100, [101,102], 7, [8,9], 10, [11,12], 20, [21,22]};          % conditions 
...
clear dyadNum condition loc_trl loc_freq col f dyad trl numOfFiles ...
      fileList srcPath data_itpc numOfTrials label data
